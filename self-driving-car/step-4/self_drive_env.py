...
class Car(Widget):
    ...
    def get_sensor_data(self, matrix, x, y):
        ...
        sensor_data = int(sensor_data) / 400.
        if sensor_data  >  0:
    ...
    def move(self, rotation):
        ...
        self.sensor1 = Vector(self.env_data.SENSOR_PROXIMITY, 0).rotate((self.angle+30)%360) + self.pos # updating the position of sensor 2
        self.sensor2 = Vector(self.env_data.SENSOR_PROXIMITY, 0).rotate(self.angle) + self.pos # updating the position of sensor 1
        ...
        self.signal1 = self.get_sensor_data(self.env_data.sand_pixel_matrix, self.sensor1_x, self.sensor1_y)
        # getting the signal received by sensor 2 (density of sand around sensor 2)
        self.signal2 = self.get_sensor_data(self.env_data.sand_pixel_matrix, self.sensor2_x, self.sensor2_y)
        # getting the signal received by sensor 3 (density of sand around sensor 3)
        self.signal3 = self.get_sensor_data(self.env_data.sand_pixel_matrix, self.sensor3_x, self.sensor3_y)
...
class SelfDriveEnv(Widget):
    ...
    def update(self, mannual=False): # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        self.env_data.screen_width = self.width
        self.env_data.screen_height = self.height
...
class SelfDriveApp(App):
    ...
    def build(self):
        ...
        env_data = SelfDriveEnvData()

        parent = SelfDriveEnv(env_data=env_data)

        self.painter = SandWidget()
        self.painter.set_env_data(env_data=env_data)

        parent.serve_car()
        Clock.schedule_interval(parent.update, 5.0 / 60.0)
